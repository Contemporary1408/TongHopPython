...
import os
import shutil
# Make function to retrieve a key's value:
def get_keyvalue(dictionary, string):
...
for i in range(2,lrow+1):
    ...
    if not os.path.exists(new_fol):
        os.mkdir(new_fol)
# Set a dict to mapping file-folder:   
dict1={}
for i in range(2,lrow+1):
    foldname = str(ws.cell(row=i,column=4).value.date().strftime("%d-%b"))
    filename = str(ws.cell(row=i,column=1).value) + " (final price).xlsx"
...
for path, dirs, files in os.walk(tempdir):
    for file in files:
        if file in dict1:
            ...
            dest_path = os.path.join(tempdir, get_keyvalue(dict1,file),file).replace("\\","/")
            try:
